...
```json
{
  "primaryTaskId": "CHRO-001",
  "parallelTracks": ["CHRO-002", "CHRO-003"],
  "dependencyGraph": {
    "CHRO-001": [],
    "CHRO-002": [],
    "CHRO-003": ["CHRO-002"]
...
```bash
node scripts/sprint-board.mjs update --id CHRO-001 --state "In Progress"
node scripts/sprint-board.mjs journal --task CHRO-001 --summary "what changed"
```
...
4. When marking a task `Failed`, record a pitfall entry alongside the state update:
   ```bash
   node scripts/sprint-board.mjs update --id CHRO-001 --state "Failed" \
     --failure-type <gate|acceptance|review> --attempted "..." --hypothesis "..."
   node scripts/sprint-board.mjs pitfall --task CHRO-001 \
     --failure-type <gate|acceptance|review> --attempted "..." --hypothesis "..." \
     [--missing-context "..."]
   ```
